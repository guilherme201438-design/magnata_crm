...
export default function EditLead() {
  ...
  useEffect(() => {
    if (lead) {
      setValue("patientName", lead.patientName);
      setValue("phone", lead.phone);
      setValue("treatmentType", lead.treatmentType);
      setValue("treatmentValue", lead.treatmentValue / 100);
      setValue("contactDate", new Date(lead.contactDate).toISOString().split("T")[0]);
      if (lead.appointmentDate) {
        setValue("appointmentDate", new Date(lead.appointmentDate).toISOString().split("T")[0]);
      }
      setValue("attended", lead.attended ?? false);
      setValue("treatmentClosed", lead.treatmentClosed ?? false);
      setValue("status", lead.status as any);
      setValue("observations", lead.observations || "");
